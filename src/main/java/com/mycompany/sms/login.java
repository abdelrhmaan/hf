...
public class login implements Serializable {
    ...
    public String Submit() throws SQLException{
        if(username != null || password != null){
            if(searchData(username, password))
                con.close();
    ...
    String mail1;
    public String sendConfirm() throws SQLException{
        
        if(email != null ){
            try {
             String query = "select * from user where mail like '"+email+"'";
             ...
             if(rs.next()){
                 ...
                 changeCode(mail1,ActivationCode);
        ...
        if(mail1 != null || confirmationCode != null || newPassword != null || reNewPassword != null){
            if(checkCode()){
                if(newPassword.equals(reNewPassword)){
                    try {
                        String query = "update user set userpassword = '"+newPassword+"' where mail = '"+mail1+"'";
                        st.execute(query);
                        msg13="Done";
                        con.close();
    ...
    public boolean checkCode() {
        try {
             String query = "select * from user where mail like '"+mail1+"'";
             rs = st.executeQuery(query);
             if(rs.next()){
    ...
    public boolean searchData(String name1 , String password1) {
		...
		boolean valid = searchData(username, password) ;
		if (valid) {
